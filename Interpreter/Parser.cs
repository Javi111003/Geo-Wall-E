...
public class Parser {
    ...
    public Parser(Lexer lexer, Context context = null, bool debug = false) {
        ...
        if (context is null) {
            this.local_context = new Context(Parser.BUILTINS);
        }
        else {
   ...
   public string ErrorMessage(Exception e) {
    ...
    public Lambda Letin() {
        ...
        // XXX this changes the references

        var temp = this.local_context.Clone();
